...
namespace Zeroit.Framework.Metro
{
	...
	public class ZeroitMetroPieChart : Control
	{
        ...
        /// The style
        /// </summary>
        private ZeroitMetroPieChart.TrackerStyle _Style;
        ...
        public ZeroitMetroPieChart()
		{
			...
			this._drawsegmentborders = false;
			this._UseDynamicBorderColors = false;
			this._SegmentBorderSize = 2;
			this._Style = ZeroitMetroPieChart.TrackerStyle.Normal;
			this._ShowSegmentNames = false;
			this._SegmentNamesPerRow = 2;
			this._UseDynamicFillColors = false;
			this._FillColorAlpha = 82;
			this._showDonutEffect = false;
			this._DonutEffectSize = 100;
			this._SeperateSegments = false;
			this._DrawBorder = false;
			this._BorderColor = Color.FromArgb(18, 173, 196);
			this._BorderSize = 2;
        ...
        protected override void OnPaint(PaintEventArgs e)
		{
			...
			Rectangle rectangle1 = new Rectangle(checked((int)Math.Round((double)this.Width / 2 - (double)this._radius / 2 - 1)), 2, this._radius, this._radius);
				...
				if (this.Style != ZeroitMetroPieChart.TrackerStyle.Abstract)
				{
					...
					for (int l = 0; l <= count2; l = checked(l + 1))
					{
						...
						using (SolidBrush solidBrush2 = new SolidBrush(this.Segments[l].FillColor))
						{
							...
							{
								Color color = this.SetShade(solidBrush2.Color, 40);
