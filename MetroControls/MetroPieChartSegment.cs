...
namespace Zeroit.Framework.Metro
{
    ...
    public class ZeroitMetroPieChartSegment : INotifyPropertyChanged
	{
		...
		{
			ZeroitMetroPieChartSegment.__ENCList = new List<WeakReference>();
		}
		...
		{
			List<WeakReference> _ENCList = ZeroitMetroPieChartSegment.__ENCList;
			Monitor.Enter(_ENCList);
			try
			{
				if (ZeroitMetroPieChartSegment.__ENCList.Count == ZeroitMetroPieChartSegment.__ENCList.Capacity)
				{
					int item = 0;
					int count = checked(ZeroitMetroPieChartSegment.__ENCList.Count - 1);
					for (int i = 0; i <= count; i = checked(i + 1))
					{
						if (ZeroitMetroPieChartSegment.__ENCList[i].IsAlive)
						{
							if (i != item)
							{
								ZeroitMetroPieChartSegment.__ENCList[item] = ZeroitMetroPieChartSegment.__ENCList[i];
							}
							item = checked(item + 1);
						}
					}
					ZeroitMetroPieChartSegment.__ENCList.RemoveRange(item, checked(ZeroitMetroPieChartSegment.__ENCList.Count - item));
					ZeroitMetroPieChartSegment.__ENCList.Capacity = ZeroitMetroPieChartSegment.__ENCList.Count;
				}
				ZeroitMetroPieChartSegment.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
			}
			finally
			{
